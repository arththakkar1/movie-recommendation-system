...
_(Note: If your data CSV files are stored elsewhere, you can point the script to that specific folder by passing the `--data-dir` argument: `python src/manage.py import_movies --data-dir /path/to/your/csv/folder`)_
...
_(Note: Cross-Origin Resource Sharing (CORS) is already pre-configured in the Django backend (`production.py` and `development.py`) to permit requests coming from `localhost:3000`.)_
